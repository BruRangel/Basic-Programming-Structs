...
int main(){
    ...
    int vetor_4[40];            // o vetor 4 pode ter no máximo 20 elementos, considerando que os vetores 1 e 2 sejam idênticos
    ...
    for(int i = 0; i < nums_2; i++){        // iterando entre todos os elementos do vetor 2
        ...
        if (!repetido){
            vetor_3[nums_1+contador_nao_rep] = vetor_2[i];  // caso o numero não seja repetido ele será adicionado ao vetor 3
            contador_nao_rep++;
        }
        else{
            // este bloco abaixo verifica se o valor repetido já está contido no vetor 4, para não exibí-lo 2 vezes
            repetido = FALSE;
    ...
    for(int i = 0; i < nums_1 + contador_nao_rep; i++){     // o número de elementos do vetor 3 é igual ao do vetor 1 + número de elementos nao repetidos no vetor 2
        printf("%d ", vetor_3[i]);
    }
